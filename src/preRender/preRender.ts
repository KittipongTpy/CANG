...
export function getPreRenderPoint(shape: string, pointList: Point[]): Point[] {
  ...
  if (shape !== "bezier") {
    if (shape === "line") {
      const result = line(
        `LIN ${pointList[0].x} ${pointList[0].y} ${pointList[1].x} ${pointList[1].y}`,
      );
    ...
    if (shape === "circle") {
      ...
      const result = circle(
        `CIR ${pointList[0].x} ${pointList[0].y} ${radius}`,
      );
    ...
    if (shape === "ellipse") {
      ...
      const result = ellipse(
        `ELI ${pointList[0].x} ${pointList[0].y} ${a} ${b}`,
      );
    ...
    if (shape === "rectangle") {
      const result1 = line(
        `LIN ${pointList[0].x} ${pointList[0].y} ${pointList[1].x} ${pointList[0].y}`,
      );
      const result2 = line(
        `LIN ${pointList[0].x} ${pointList[0].y} ${pointList[0].x} ${pointList[1].y}`,
      );
      const result3 = line(
        `LIN ${pointList[1].x} ${pointList[0].y} ${pointList[1].x} ${pointList[1].y}`,
      );
      const result4 = line(
        `LIN ${pointList[0].x} ${pointList[1].y} ${pointList[1].x} ${pointList[1].y}`,
      );
      const points = [result1, result2, result3, result4]
        .filter(
          (res): res is { type: "line"; points: [number, number][] } =>
            typeof res === "object" && res?.type === "line",
        )
        .flatMap((res) =>
          res.points.map((point: [number, number]) => ({
            x: point[0],
            y: point[1],
          })),
        );
    ...
    if (shape === "hermite") {
      const result = hermite(
        `HER ${pointList[0].x} ${pointList[0].y} ${pointList[3].x} ${pointList[3].y} ${pointList[1].x} ${pointList[1].y} ${pointList[2].x} ${pointList[2].y}`,
      );
